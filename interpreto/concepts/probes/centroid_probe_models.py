...
class _BaseCentroidProbe(ProbeModelInterface):
    ...
    """
    def __init__(self, normalization: NormalizationBase | None = None, eps: float = 1e-8):
        super().__init__()
        self.eps = float(eps)
...
class CosineCentroidCentroidProbe(_BaseCentroidProbe):
    """
...
class SqL2CentroidProbe(_BaseCentroidProbe):
    ...
    @assert_fitted
    def encode(self, X: torch.Tensor) -> torch.Tensor:
        Xn = self.normalization(X) if self.normalization else X     # (n, d)
...
class DiagonalMahalanobisCentroidProbe(_BaseCentroidProbe):
    ...
    def __init__(self, normalization: NormalizationBase | None = Standardization(), eps: float = 1e-8, shrinkage: float = 1.0):
